...
function importFromMarkdown(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            const existing = new Set(state.records.map(r => r.startTime + '|' + r.duration));
            const newRecs = records.filter(r => !existing.has(r.startTime + '|' + r.duration));
...
function parseMarkdown(content) {
    ...
    let pending = null; // 暂存当前记录，等待后续注释/标签行补齐

    function commit() {
        if (pending) { records.push(pending); pending = null; }
    }

    lines.forEach(line => {
        const dateMatch = line.match(/## (\d{4})[-/](\d{1,2})[-/](\d{1,2})/);
        if (dateMatch) { commit(); currentDate = new Date(dateMatch[1], dateMatch[2] - 1, dateMatch[3]); return; }
        const commentMatch = line.match(/^> (.*)$/);
        if (commentMatch && pending) { pending.comment = commentMatch[1]; return; }
        const tagMatch = line.match(/^\*\*Tags:\*\* (.+)$/);
        if (tagMatch && pending) { pending.tags = tagMatch[1].split(',').map(t => t.trim()).filter(Boolean); return; }
        const match = line.match(timePattern);
        if (match && currentDate) {
            commit(); // 新记录行到来，先把上一条落库
